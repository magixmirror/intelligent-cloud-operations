...
from intops_utils import load_run_data, make_clf_input_pairs, make_data_chunks
...
train_data_list = sorted(
    list(default_data_root.glob("distwalk-disk-load-0[4-6]"))
    + list(default_data_root.glob("distwalk-disk-load-1[0-6]"))
    + list(default_data_root.glob("distwalk-disk-load-2[1-5]"))
    + list(default_data_root.glob("distwalk-disk-load-3[0-9]"))
)
test_data_list = sorted(
    list(default_data_root.glob("distwalk-disk-load-0[7-9]"))
    + list(default_data_root.glob("distwalk-disk-load-1[7-9]"))
    + list(default_data_root.glob("distwalk-disk-load-20"))
    + list(default_data_root.glob("distwalk-disk-load-2[6-9]"))
    + list(default_data_root.glob("distwalk-disk-load-4[0-1]"))
)
...
for data_idx, run_data in enumerate(run_data_list):
    load_df, times_df, injection_df, anomaly_type = load_run_data(run_data)
...
model_dump_filename = f"clf_xgboost_{current_date}.joblib"
bst.save_model(model_dump_filename)
print(model_dump_filename)

scaler_dump_filename = f"clf_scaler_{current_date}.joblib"
...
ax.grid()
fig.savefig(default_img_dest / "clf_roc-auc.eps", bbox_inches="tight")
...
fig.savefig(default_img_dest / "clf_conf-mat.eps", bbox_inches="tight")
